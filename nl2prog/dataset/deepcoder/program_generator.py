...
from nl2prog.dataset.deepcoder.dsl \
    import Function, Variable, Expression, Program, Statement
...
def arguments(id_generator: IdGenerator, variables: Set[Variable], signature):
    ...
    ------
    ArgumentWithState
        The argument list and the state that will be used to continue
        enumeration
    """
        ...
        if len(elem.arguments) == len(signature):
            ...
            for v in candidates:
                ...
                s.append(ArgumentWithState(
                    [*(elem.arguments), v], elem.generator, elem.variables,
                    elem.new_variables))
            # Create new var
            ...
            s.append(ArgumentWithState(
                arg_new, generator_new, vars_new,
                [*(elem.new_variables), v_new]))
    ...
    while len(s) != 0:
        ...
        for func in functions:
            ...
            for a in arguments(g, vars, func.signature.input_types):
                ...
                p_new.body.append(
                    Statement(Variable(generator.generate(),
                                       func.signature.output_type),
                              Expression(func, a.arguments)))
                s.append((p_new, generator))


def random_programs(functions: List[Function], min_length: int,
                    max_length: int,
