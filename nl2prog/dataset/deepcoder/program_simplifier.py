from nl2prog.dataset.deepcoder.dsl \
    import Function, Expression, Program, Statement
...
def remove_redundant_expressions(program: Program) -> Program:
    ...
    body = []
    for statement in program.body:
        if (statement.expression.function.name,
            tuple(statement.expression.arguments)) in \
                expression_to_variable:
            # Rule1
            replacement[statement.variable] = expression_to_variable[(
                statement.expression.function.name,
                tuple(statement.expression.arguments))]
            continue
        if len(statement.expression.arguments) > 0 and \
                statement.expression.arguments[0] in variable_to_expression:
            exp_arg1 = \
                variable_to_expression[statement.expression.arguments[0]]
            if statement.expression.function.name == "SORT" and \
                    (exp_arg1.function.name == "SORT"):
                # Rule2
                replacement[statement.variable] = \
                    statement.expression.arguments[0]
                continue
            if statement.expression.function.name == "REVERSE" and \
                    (exp_arg1.function.name == "REVERSE"):
                # Rule3
                replacement[statement.variable] = \
                    variable_to_expression[statement.expression.arguments[0]] \
                    .arguments[0]
...
def remove_dependency_between_variables(program: Program, minimum: Function,
    ...
    for statement in program.body:
        if len(statement.expression.arguments) > 0 and \
            ...
            exp_arg1 = \
                variable_to_expression[statement.expression.arguments[0]]
            if (exp_arg1.function.name == "SORT" or
                exp_arg1.function.name == "REVERSE") and \
                    (statement.expression.function.name == "SUM" or
                     statement.expression.function.name == "MAXIMUM" or
                     statement.expression.function.name == "MINIMUM"):
                # Rule1
                x = exp_arg1.arguments[0]
                exp = Expression(statement.expression.function, [x])
                body.append(Statement(statement.variable, exp))
                variable_to_expression[statement.variable] = \
                    statement.expression
                continue
            if exp_arg1.function.name == "SORT" and \
                    (statement.expression.function.name == "HEAD"):
                # Rule2
                x = exp_arg1.arguments[0]
                ...
                continue
            if exp_arg1.function.name == "SORT" and \
                    (statement.expression.function.name == "LAST"):
                # Rule3
                x = exp_arg1.arguments[0]
