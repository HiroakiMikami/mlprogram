...
import copy
from nl2prog.dataset.deepcoder.deepcoder_utils import generate_io_samples
...
class Signature:
    ...
    def __eq__(self, rhs):
        return self.input_types == rhs.input_types and \
...
class Function:
    ...
    def __hash__(self):
        return hash((self.name,
                     (tuple(self.signature.input_types),
...
class Program:
    ...
    def to_string(self) -> str:
        ...
        for input in self.inputs:
            code += "{} <- {}\n".format(
                id_to_name(input.id), "int" if input.t == Type.Int else "[int]"
            )
        for statement in self.body:
            code += "{} <- {} {}\n".format(
                id_to_name(statement.variable.id),
                statement.expression.function.name,
                " ".join(map(
                    lambda x: id_to_name(x.id), statement.expression.arguments)
                ))
    ...
    def clone(self):
        ...
        for statement in self.body:
            ...
            body.append(Statement(copy.deepcopy(statement.variable),
                                  Expression(statement.expression.function,
                                             args)))
